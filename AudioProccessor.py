...
class AudioProccessor(object):
    ...
    def get_speech(self, seconds_flag: bool = True):
        speech_timestamps = self.functions['get_speech_timestamps'](self.wav, self.model, return_seconds=seconds_flag)
        return [(x['start'], x['end']) for x in speech_timestamps]
    ...
    def get_silent(self, noise_vector: list):
        ...
        for i in range(len(noise_vector)-1):
            ...
            silent_vector.append((noise_vector[-1][1], round(self.duration, 1)))

        return silent_vector
